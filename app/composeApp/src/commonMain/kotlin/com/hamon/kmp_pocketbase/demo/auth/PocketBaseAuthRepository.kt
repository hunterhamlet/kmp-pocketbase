...
internal class PocketBaseAuthRepository(private val pb: PocketBase) : AuthRepository {
    private val service = pb.collection("users")

    override val isLoggedIn: Boolean get() = pb.authStore.isValid

    override suspend fun login(email: String, password: String): PocketBaseResult<AuthResponse<UserRecord>> =
        service.tryAuthWithPassword(email, password)
    ...
    override suspend fun refresh(): PocketBaseResult<AuthResponse<UserRecord>> =
        service.tryAuthRefresh()
